...
const TimerListScreen = ({ navigation }) => {
  ...
  const renderTimer = ({ item }) => (
    <View style={[styles.timerItem, isDarkMode && styles.timerItemDark]}>
      <View style={styles.timerHeader}>
        <View style={styles.timerInfo}>
          <Text style={[styles.timerName, isDarkMode && styles.timerNameDark]}>{item.name}</Text>
          <Text style={[styles.statusText, styles[item.status?.toLowerCase() || 'paused'], isDarkMode && styles.statusTextDark]}>
            {item.status || 'Paused'}
          </Text>
        </View>
        <View style={styles.alertContainer}>
          <Text style={isDarkMode && styles.textDark}>Halfway Alert</Text>
          <Switch
            value={item.halfwayAlert}
            onValueChange={() => toggleHalfwayAlert(item.id)}
          />
        </View>
      </View>
        ...
        <View style={styles.progressBar}>
          <View
            style={[
              styles.progressFill,
              {
                width: `${calculateProgress(item.remainingTime, item.duration)}%`,
                backgroundColor: item.status === 'Completed' ? '#4CAF50' : '#2196F3',
              },
            ]}
          />
        </View>
        <Text style={[styles.percentText, isDarkMode && styles.percentTextDark]}>
          {Math.round(calculateProgress(item.remainingTime, item.duration))}%
        </Text>
      </View>
      <View style={styles.controls}>
        <TouchableOpacity
          style={[styles.button, item.status === 'Running' && styles.buttonDisabled, isDarkMode && styles.buttonDark]}
